...
from lotto.utils import *
from lotto.updater import Updater
from lotto.predictor import *
...
class Lotto:
    ...
    def __init__(self, argv):
        Lotto.lt = self
        parser = argparse.ArgumentParser(description="Manage a database and process orders.")
        parser.add_argument("--db", help="Path to a file containing real lotto numbers")
        parser.add_argument("--orders", help="Path to a file containing orders")
        parser.add_argument("--plot", action='store_true', help="Path to a file containing orders")
    ...
    def _execute(self, args):
        ...
        # Load database
        self.history_db = load_db(args.db)
        self.orders_db = load_db(args.orders)

        # Update database
        updater = Updater()
        while difference := updater.fetch(self.history_db):
            self.history_db = update_db(args.db, self.history_db, difference)
            time.sleep(1)
    ...
    def _plot(self):
        ...
        fig.canvas.setWindowTitle(VERSION)
        ...
        for x in range(len(axes)):
            ...
            axes[x].set_ylim([self.statistics.get_mins()[x]-2, self.statistics.get_maxs()[x]+2])
        ...
        if self.opt_plot_orders_db:
            ...
            for x in range(len(axes)):
                axes[x].fill_between(xticks[:], [self.statistics.get_devianceplus()[x]] * (tickcount),
                                     [self.statistics.get_devianceminus()[x]] * (tickcount), facecolor='green', alpha=0.45, zorder=3)
                axes[x].plot(xticks[:], [self.statistics.get_means()[x]]*(tickcount), zorder=3)
                        ...
                        for y in range(len(realdata)):
                            for j in range(len(self.orders_db[key])):
                                ...
                                if j >= len(hitticks[y]):      # create if not present
                                    ...
                                    scatters_order_data[y].append(axes[y].scatter(
                                        [], [], c="red", s=50, cmap=cmap, marker="o", vmin=0, vmax=10))
...
@app.route('/script.js')
def script():
    keys = list(Lotto.lt.history_db.keys())
    points = convert_db_to_points(Lotto.lt.history_db)
    accumulative = Lotto.lt.statistics.get_accumulative()
    numbers_ratio = Lotto.lt.statistics.get_numbers_ratio()

    matched = Lotto.lt.get_matched_numbers()
    return render_template('script.js',
                           range1=[int(Lotto.lt.statistics.get_devianceminus()[0]),
                                   int(Lotto.lt.statistics.get_devianceplus()[0])],
                           range2=[int(Lotto.lt.statistics.get_devianceminus()[1]),
                                   int(Lotto.lt.statistics.get_devianceplus()[1])],
                           range3=[int(Lotto.lt.statistics.get_devianceminus()[2]),
                                   int(Lotto.lt.statistics.get_devianceplus()[2])],
                           range4=[int(Lotto.lt.statistics.get_devianceminus()[3]),
                                   int(Lotto.lt.statistics.get_devianceplus()[3])],
                           range5=[int(Lotto.lt.statistics.get_devianceminus()[4]),
                                   int(Lotto.lt.statistics.get_devianceplus()[4])],
                           x1_values=keys,
